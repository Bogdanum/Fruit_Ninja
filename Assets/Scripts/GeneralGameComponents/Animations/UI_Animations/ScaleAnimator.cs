...
public class ScaleAnimator : MonoBehaviour
{
    ...
    public void ShowWithDelay(UISettings settings)
    {
        _tween = transform.DOScale(Vector3.one, settings.gameOverPopupFadeInDuration).SetEase(showEase).SetDelay(settings.timeToShowGameOverPopup);
        _tween.OnComplete(() =>
        {
            onCompleteShow?.Invoke();
            _tween.Kill();
